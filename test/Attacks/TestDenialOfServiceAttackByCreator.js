...
const DOSAttacker = artifacts.require("./TestContracts/DOSAttacker.sol");



contract("TestDenialOfServiceAttackByCreator", accounts => {
  ...
  it("set creator to a malicious user", async () => {

    malCreator = await DOSAttacker.new(holoNFT.address,morpheuseMarket.address);
    creator = malCreator.address;
  ...
  it("collectorB buys collectorA's item (with signature)", async () => {
    ...
    assert.deepEqual(creatorBalance.toString(), newCreatorBalance.toString(), "creator Balance balance incorrect");

    let marketBalance = web3.utils.toBN(String(await web3.eth.getBalance(morpheuseMarket.address)));
    assert.deepEqual(marketBalance.toString(), creatorAmount.toString(), "contract Balance balance incorrect");

  })

  it("creator withdraw credits afterward", async () => {

    let creatorBalance = web3.utils.toBN(String(await web3.eth.getBalance(creator)));

    await malCreator.claimCredit();
